...
export const StockStyleDashboard = ({
  ...
  return (
    <div className="space-y-6">
      {/* Stock Ticker Header */}
      <Card className="bg-gradient-to-br from-blue-500/10 to-purple-500/10 border-blue-200/50 dark:border-blue-800/50">
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart data={chartData}>
              ...
              <YAxis
                tick={{ fontSize: 12, fill: textColor }}
                label={{ value: 'Triệu VND/m²', angle: -90, position: 'insideLeft', fill: textColor }}
                stroke={gridColor}
              />
              <Tooltip
              ...
              <ReferenceLine
                ...
                strokeDasharray="3 3"
                label={{ value: 'IPO', position: 'right', fill: textColor }}
              />
              <Area
                type="monotone"
      ...
      <Tabs defaultValue="upcoming" className="w-full">
        ...
        <TabsContent value="upcoming" className="space-y-4 mt-4">
          {expectedPriceImpact > 0 && (
            <Card className="bg-green-50/50 dark:bg-green-950/20 border-green-200/50 dark:border-green-900/50">
              <CardContent className="p-4 flex items-center gap-3">
                <TrendingUp className="h-8 w-8 text-green-600" />
                <div>
                  <h4 className="font-bold text-lg text-foreground">Expected Price Impact: +{expectedPriceImpact.toFixed(1)}%</h4>
          ...
          {upcomingCatalysts.length === 0 ? (
            ...
            upcomingCatalysts.map((catalyst) => (
              <Card key={catalyst.id} className="hover:shadow-md transition-shadow border-border/50">
                <CardContent className="p-6">
        ...
        <TabsContent value="past" className="space-y-4 mt-4">
          {pastCatalysts.length === 0 ? (
            ...
            pastCatalysts.map((catalyst) => (
              <Card key={catalyst.id} className="opacity-80 bg-muted/30 border-border/50">
                <CardContent className="p-6">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid md:grid-cols-3 gap-4">
            <div className="text-center p-4 bg-green-50/50 dark:bg-green-950/20 rounded-lg border border-green-100 dark:border-green-900/50">
