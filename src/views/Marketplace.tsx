  ...
  const content = (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button size="sm" onClick={() => navigate.push('/marketplace/create')} className="md:w-auto w-full shadow-lg shadow-primary/20">
...
function ListingCard({ listing, onToggleFavorite }: ListingCardProps) {
  ...
  return (
    <Card className="group hover:shadow-md transition-all duration-300 overflow-hidden bg-card border-border h-full flex flex-col">
      <Link href={`/marketplace/listing/${listing.id}`} className="relative aspect-[4/3] overflow-hidden bg-muted block">
        <img
          src={primaryImage}
          alt={listing.title}
      ...
      <CardContent className="p-3 flex flex-col flex-1">
        <Link href={`/marketplace/listing/${listing.id}`} className="block flex-1">
