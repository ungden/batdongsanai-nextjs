...
const ProjectDetail = () => {
  ...
  const { project, loading } = useProjectDetail(id as string);
  const { catalysts } = useMarketCatalysts({ projectId: id as string }); // Fetch catalysts
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate.push('/projects')} className="rounded-xl">
  ...
  const handleCalculate = () => {
    navigate.push(`/calculator?price=${project.pricePerSqm}&area=70`);
  };
